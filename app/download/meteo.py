...
url_seed = "http://meteosearch.meteo.gr/data/"
data_folder = os.path.join(basedir, 'app/tmp')
...
class MeteorologicalDataDownloader:
    ...
    def __init__(self, year_from=2014, year_to=2015, location='crete'):
        ...
        self.path = os.path.join(basedir, 'app/locations')+'/'+self.location+'.txt'
        self.stations = pd.read_csv(self.path)  # in this location there are stations
    ...
    def download_file(self):
        ...
        # http://meteosearch.meteo.gr/data/agiakiriaki/2011-10.txt
        """
        for station in self.stations['stations'][:]:
            flash(station)
            print(data_folder)
            try:
                os.mkdir(data_folder+'/'+station)
            ...
            for i, date in enumerate(self.dates_to_download):
                ...
                try:
                    ...
                    testfile.retrieve(url, data_folder+'/' + station + '/' + station + '-' + date + '.txt')
                except:
                    pass
            # os.chdir(os.pardir)
            # os.chdir(os.pardir)
    ...
    def main(self):
        if os.path.exists(os.path.join(basedir, 'app/locations')):
            print(os.path.join(basedir, 'app/locations'))
        else:
            print('loosing it!')
...
if __name__ == "__main__":
    mdd = MeteorologicalDataDownloader(2014, 2017, location='crete')
    mdd.main()
